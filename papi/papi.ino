...
#define MODE_LAMP        3
#define MODE_FULL_LANP   4
#define _MODE_END        5

#define TWINKLE   5

int pins[]       = {CLASS1, CLASS2, CLASS3, CLASS4, CLASS5};
int brightness[] = {255, 224, 192, 158, 126};
int off[]        = {0, 0, 0, 0, 0};
...
void setup() {
  for (int i=0; i<CLASSES; ++i) pinMode(pins[i], OUTPUT);
  pinMode(LAMP, OUTPUT);
...
float getFactor() {
  float factor(analogRead(CONTROL)*1.0/900.0);
...
void loop() {
  switch (mode) {
    ...
    case MODE_TWINKLE: {
      ...
      for (int i=0; i<CLASSES; ++i) current[i] = brightness[i] * factor;
  
      if (MODE_TWINKLE == mode) {
        for (int i=0; i<CLASSES; ++i) {
          int value(current[i] + random(-TWINKLE, TWINKLE));
    ...
    case MODE_FULL_LANP: {
      float factor(getFactor());
      for (int i=0; i<CLASSES; ++i) current[i] = 255.0 * factor;
      setLeds(current);
      analogWrite(LAMP, 255.0 * factor);
      break;
    }
  }
